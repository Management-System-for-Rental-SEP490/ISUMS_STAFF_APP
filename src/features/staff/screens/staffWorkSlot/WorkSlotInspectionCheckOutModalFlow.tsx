...
export type CheckInBaselineEntry = {
  conditionPercent?: number | null;
  note?: string | null;
};

export type WorkSlotInspectionCheckOutModalFlowProps = {
  t: TFunction;
  insetsTop: number;
  ...
  selectedMaintenanceAssetId: string | null;
  checkInBaselineByAssetId: Record<string, CheckInBaselineEntry>;
...
export function WorkSlotInspectionCheckOutModalFlow(props: WorkSlotInspectionCheckOutModalFlowProps) {
  const {
    t,
    insetsTop,
    currentHouseName,
  ...
  const baseline =
    selectedMaintenanceAssetId && checkInBaselineByAssetId[selectedMaintenanceAssetId]
      ? checkInBaselineByAssetId[selectedMaintenanceAssetId]
      : null;
              ...
              {maintenanceEditorLoading ? (
                ...
                {baseline ? (
                  <Text style={[M.maintenanceHintText, M.baselineHintMargin]}>
                    {t("staff_work_slot_detail.baseline_checkin", {
                      percent: String(baseline.conditionPercent ?? "—"),
                      note: baseline.note?.trim() || "—",
                    })}
                  </Text>
                ) : null}
